...
const Movie = (props) => {
  ...
  const favorites = movies.find((movie) => movie.id === Number(id));
  ...
  const addSubmit = (e) => {
    e.preventDefault();
    dispatch(FavoriteMovieList(id));
    push("/FavoriteMovieList");
  };
  return (
    <div className="bg-white rounded-md shadow flex-1">
      ...
      <div className="px-5 py-3 border-t border-zinc-200 flex justify-end gap-2">
        ...
        <button
          type="button"
          onClick={addSubmit}
          className="myButton bg-green-600 hover:bg-blue-600 "
        >
          Favorilere ekle
        </button>
      </div>
